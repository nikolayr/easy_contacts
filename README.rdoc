...
2do
 add .identifier to Project.find() route
 add author_id to contact model
 rename activity_ec_title to Contact #%{id}: %{first_name} %{last_name}
 romove puts in def easy_contact_path
 restore custom_field_values and custom_fields vars in controller
 add custom fields
 add custom query to contacts#index
 add pagination

 add custom fields management to plugin settings page
